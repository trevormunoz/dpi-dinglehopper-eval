...
def test_run_page_passes_differences_flag(tmp_path, monkeypatch):
    """Engine-fence amendment 6: run_page also passes dinglehopper's
    --differences option (a click BOOL VALUE option, not a bare flag —
    verified empirically against the pinned version) so per-page JSON
    reports carry character/word difference data and
    dinglehopper-summarize can build its common-mistakes tables.
    """
    gt = PureWindowsPath("C:/Users/student/eval/gt/page_0.gt.txt")
    ocr = PureWindowsPath("C:/Users/student/eval/ocr/page_0.txt")
    reports_dir = tmp_path / "reports"
